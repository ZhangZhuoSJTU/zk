#!/bin/bash
# Compile, then run all ProgramBench test branches for zk-org__zk.10d93d5.
#
...
while read -r branch; do
    ...
    mkdir -p "$bindir"
    # macOS BSD `script` lacks GNU's `-c`; several suites fake a TTY with
    # `script -q -c CMD FILE`. Shim it to the BSD form so those tests run.
    cat > "$bindir/script" <<'SHIM'
#!/bin/bash
# BSD script emits a leading "^D\b\b" EOF echo that GNU script does not;
# strip it so captured output matches goldens. Preserve script's exit status.
q=""; cmd=""; file="/dev/null"
while [ $# -gt 0 ]; do
  case "$1" in
    -q) q="-q"; shift ;;
...
strip() { perl -0777 -pe 's/\A\x5e\x44\x08\x08//'; }
if [ -n "$cmd" ]; then
  /usr/bin/script $q "$file" /bin/sh -c "$cmd" | strip
  exit ${PIPESTATUS[0]}
fi
/usr/bin/script $q "$file" "$@" | strip
exit ${PIPESTATUS[0]}
SHIM
    ...
    echo "=== branch $branch ==="
    # TMPDIR=/private/tmp (already canonical) so zk's path canonicalization does
    # not turn the tests' /tmp temp dirs into /private/tmp and mismatch goldens.
    (cd "$dir" && TMPDIR=/private/tmp PATH="$bindir:$PATH" bash eval/run.sh > run.log 2>&1 || true)
